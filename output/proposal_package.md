...
## 第4章: Excel実データ vs モックアップの精度検証（検証完了）
...
|------|-------------|-------------|------|
| 営繕費 113.3% | デモデータ | 営繕費: 316,512/539,588 = 58.7% | **不一致（デモデータ）** → モックアップ修正済み |
| 仮設材料費 80.2% | デモデータ | 該当費目なし（28コード体系に存在しない） | **不一致（架空費目）** → モックアップ修正済み |
| 土木一般世話役 76.2% | デモデータ | 該当費目なし | **不一致（架空費目）** → モックアップ修正済み |
| 機械損料 72.4% | デモデータ | 機械経費損料: 0/137,440 = 0% | **不一致（デモデータ）** → モックアップ修正済み |
...
モックアップでは経費コードが「A-101」「B-201」「C-301」等の架空コードで、費目名も「営繕費」「仮設材料費」「土木一般世話役」「機械損料」等のデモデータ。**修正前は不一致であったが、モックアップ修正済み。**
...
モックアップ（Sheets）には相殺処理のデモデータがあるが、上記の実データに基づきモックアップ修正済み。
...
| 箇所 | 修正前 | 対応状況 |
|------|------|---------|
| page1 予算超過アラートテーブル | 架空データ（営繕費113.3%等） | 対応済み: Excel実データに基づく3費目に差替 |
| page2 予実対比テーブル | 架空コード・架空データ | 対応済み: 28コード体系の実データに差替 |
| page2 予算vs実績グラフ | 架空データ | 対応済み: 実データに基づくグラフデータに差替 |
| page4 AIチャット応答 | 架空数値（外注費2,156,000円等） | 対応済み: Excel実データの数値を反映 |

#### Sheets mockup（mockup_sheets.html）

| 箇所 | 修正前 | 対応状況 |
|------|------|---------|
| Tab1 支払明細入力 | デモデータ（川越建設3,200,000円等） | 対応済み: Excel実データに差替 |
| Tab2 予算設定 | 架空コード（A01/B01/C01等） | 対応済み: 28コード体系に差替 |
| Tab2 費目名 | 架空名（土木一般外注費/コンクリート材料費等） | 対応済み: 正式名称に差替 |
...
|-----|-----------|-------|---------|------|
| 1 | config.gs | 14KB | 434 | シート保護/メニュー/定数/前行コピー/入力期限通知 |
| 2 | template.gs | 39KB | 1,125 | テンプレート自動生成（available_elements対応/2段階入力/18列） |
| 3 | validation_extended.gs | 17KB | 466 | カスケードDD/数量列制御/予算箱ID解決 |
| 4 | aggregation.gs | 18KB | 539 | 月次集計/費目別集計/スナップショット方式 |
| 5 | budget_health.gs | 20KB | 617 | 予実管理計算（消化率/出来高率/過不足見込み/信号判定） |
| 6 | api.gs | 29KB | 717 | Web App API（5モード: health/master/summary/project/aggregate） |
| 7 | hub.gs | 18KB | 573 | 本社管理台帳（複数現場横断、予算健康度連携） |
...
**Phase Aの実装状況**: GAS 7ファイル（約157KB、4,471行）の開発は完了。以下はデプロイ以降の計画。
